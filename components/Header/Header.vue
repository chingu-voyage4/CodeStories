    ...
    <v-toolbar prominent app color="header-bg" class="header-content" height="75px">
      ...
      <v-toolbar-title class="hidden-md-and-up">
        <v-btn flat to="/" style="color:transparent">
          <img class="logo" src="~assets/images/CHINGU_LOGO-07_crop.png" alt="CodeStories logo" align="middle" />
        </v-btn>
      </v-toolbar-title>
      <v-spacer></v-spacer> 
      <router-link to="/">     
        <v-btn icon>
          <v-avatar size="125px" class="hidden-sm-and-down">
            <img src="~assets/images/CHINGU_LOGO-04.png" alt="CodeStories logo" align="middle" />
          </v-avatar>
        </v-btn>
      </router-link>
      <v-spacer></v-spacer>
      ...
      <v-toolbar-items class="hidden-sm-and-down" v-else>          
          ...
          <v-btn class="buttons" flat to="/signup">Join</v-btn>
      </v-toolbar-items>
    </v-toolbar>
  </header>
</template>
...
export default {
  ...
  computed: mapGetters({
    user: "user",
    singleStory: "singleStory/GET_SINGLE_STORY"
  }),
  ...
  methods: {
    ...
    handleEdit() {
      ...
      this.$store.commit("MUTATE_SINGLE_STORY", {
        key: "onSinglePage",
        val: false
      });
